...
class CustomSignupForm(forms.Form):
    ...
    birth_date = forms.DateField(input_formats=('%Y/%m/%d', '%Y-%m-%d', '%m/%d/%Y', '%m-%d-%Y'), widget=forms.TextInput(attrs={'type':'date'}))
    phone = forms.CharField(max_length=15, required=False, widget=forms.TextInput(attrs={'placeholder':'Phone Number'}))
    height_feet = forms.IntegerField(max_value=99, widget=forms.TextInput(attrs={'placeholder':'Height (Feet)'}))
    height_inches = forms.DecimalField(max_value=11.99, decimal_places=2, widget=forms.TextInput(attrs={'placeholder':'Height (Inches)'}))
    weight_lbs = forms.IntegerField(max_value=999, widget=forms.TextInput(attrs={'placeholder':'Weight (lbs)'}))
